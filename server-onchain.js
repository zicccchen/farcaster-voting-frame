...
function getContractWithSigner(privateKey) {
    ...
    return new ethers.Contract(process.env.VOTING_CONTRACT_ADDRESS, VOTING_ABI, wallet);
}
...
app.post("/api/vote", async (req, res) => {
    try {
        ...
        const proposalId = 1;
        const supporterAddress = untrustedData?.fid ? `0x${untrustedData.fid}` : null;

        const contract = getContract();

        if (buttonIndex === 1) {
            // 赞成票 - 生成交易数据
            const voteData = contract.interface.encodeFunctionData("vote", [proposalId, true]);

            res.json({
                type: "frame",
                frameUrl: `${process.env.FRAME_URL}/api/vote-confirm?support=true&proposalId=${proposalId}`,
                // 返回交易数据让用户签名
                transaction: {
                    to: process.env.VOTING_CONTRACT_ADDRESS,
                    data: voteData,
                    value: "0"
                }
            });
            ...
            // 反对票 - 生成交易数据
            const voteData = contract.interface.encodeFunctionData("vote", [proposalId, false]);

            res.json({
                type: "frame",
                frameUrl: `${process.env.FRAME_URL}/api/vote-confirm?support=false&proposalId=${proposalId}`,
                transaction: {
                    to: process.env.VOTING_CONTRACT_ADDRESS,
                    data: voteData,
                    value: "0"
...
app.get("/api/vote-confirm", async (req, res) => {
    try {
        ...
        <svg width="600" height="400" xmlns="http://www.w3.org/2000/svg">
            ...
            <text x="300" y="80" font-family="Arial" font-size="24" font-weight="bold" fill="#333" text-anchor="middle">
                确认${voteType}票
            </text>

            <text x="300" y="110" font-family="Arial" font-size="14" fill="#666" text-anchor="middle">
                ${titleStr.substring(0, 50)}${titleStr.length > 50 ? '...' : ''}
            </text>

            <rect x="100" y="150" width="400" height="80" rx="10" fill="#f3f4f6"/>
            <text x="300" y="185" font-family="Arial" font-size="16" fill="#333" text-anchor="middle">
                你将投出${voteType}票
            </text>
            <text x="300" y="210" font-family="Arial" font-size="12" fill="#666" text-anchor="middle">
                此操作需要在 Base 链上签名并支付少量 gas
            </text>

            <rect x="150" y="260" width="300" height="40" rx="20" fill="#8b5cf6"/>
            <text x="300" y="288" font-family="Arial" font-size="14" font-weight="bold" fill="white" text-anchor="middle">
                点击下方按钮完成投票
            </text>

            <text x="300" y="330" font-family="Arial" font-size="12" fill="#999" text-anchor="middle">
                在 Farcaster Cast 中点击完成投票
            </text>
        </svg>
        `;
        ...
        <head>
            ...
            <meta property="fc:frame:button:1" content="✅ 确认投票" />
            <meta property="fc:frame:button:1:action" content="tx" />
            <meta property="fc:frame:button:1:target" content="${process.env.VOTING_CONTRACT_ADDRESS}" />
            <meta property="fc:frame:button:1:tx:data" content="${contract.interface.encodeFunctionData('vote', [proposalId, support === 'true'])}" />
        </head>
        </html>
        `;
...
app.get("/api/result", async (req, res) => {
    try {
        ...
        let message = "📊 投票结果";
        if (support === "true") message = "✅ 赞成票已记录!";
        if (support === "false") message = "❌ 反对票已记录!";
...
app.get("/api/check-vote", async (req, res) => {
    try {
        const { proposalId, address } = req.query;
        ...
        const contract = getContract();
        const hasVoted = await contract.hasVoted(proposalId || 1, address);
...
app.listen(PORT, () => {
    console.log(`\n🚀 投票服务器运行在 http://localhost:${PORT}`);
    console.log(`📄 查看 Frame: http://localhost:${PORT}`);
    console.log(`\n📝 智能合约地址: ${process.env.VOTING_CONTRACT_ADDRESS}`);
    console.log(`⛽ 网络类型: ${process.env.RPC_URL?.includes('base.org') ? 'Base 主网' : '本地网络'}\n`);
});
